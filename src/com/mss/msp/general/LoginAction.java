/*
 * @(#)LoginAction.java 1.0 Nov 01, 2015
 *
 * Copyright 2015 Miracle Software Systems(INDIA) Pvt Ltd. All rights reserved.
 *
 */
...
/**
 * The <code>LoginAction</code> class is used for to enter in to ServicesBay
 * from <i>Login.jsp</i> page.
 *
 * @author Prasad kandregula <a href=
 *         "mailto:vkandregula@miraclesoft.com">vkandregula@miraclesoft.com</a>
 *
 * @version 1.0 Nov 01, 2015
 *
...
public class LoginAction extends ActionSupport implements ServletRequestAware, ServletResponseAware {
	 ...
	 * authenticated person or not, user enter loginId and password in login
	...
	public String execute() throws Exception {
		...
		try {
			...
			if (DataSourceDataProvider.getInstance().getOrgIdByEmailExt(getEmailId().toLowerCase().trim()) > 0) {
				String SQL_QUERY = "SELECT u.usr_id as usrId,login_id,salt,PASSWORD,type_of_user,first_name,last_name,CONCAT(LEFT(first_name , 1),LEFT(last_name , 1)) AS ImgAlt ,"
						...
						+ " LEFT OUTER JOIN accounts acc ON (org_id=acc.account_id) WHERE acc.STATUS='Active' and login_id like '"
						+ getEmailId().toLowerCase().trim() + "'";
				...
				while (resultSet.next()) {
					...
					dbImageAlt = resultSet.getString("ImgAlt");
					...
					if (resultSet.getString("living_country") != null) {
						dbLivingCountry = com.mss.msp.util.DataSourceDataProvider.getInstance()
								.getCountry(resultSet.getInt("living_country"));
					}
					if (resultSet.getString("working_country") != null) {

						dbWorkingCountry = com.mss.msp.util.DataSourceDataProvider.getInstance()
								.getCountry(resultSet.getInt("working_country"));
					}
				...
				int accType = DataSourceDataProvider.getInstance().checkUserIsCustomerOrVendor(dbOrgId);
				System.out.println("accType-->" + accType);

				// if(!(accType).equlas(getCustomerOrVenFlag())
				System.out.println("getCustomerOrVenFlag--->" + getCustomerOrVenFlag());
				if (getCustomerOrVenFlag() == 5) {
					errorMessage = "vendorErrorMessage";
				} else if (getCustomerOrVenFlag() == 0) {
					errorMessage = "csrOrAdminErrorMessage";
					setCustomerOrVenFlag(1);
				} else {
					errorMessage = "customerErrorMessage";
				}

				if (accType != getCustomerOrVenFlag()) {
					getHttpServletRequest().setAttribute(errorMessage,
							"<font color=\"red\" size=\"1.5\">Please Login with valid UserId and Password! </font>");
					resultType = INPUT;
				...
				if (!"Inactive".equalsIgnoreCase(dbAccStatus)) {
					if ("Registered".equalsIgnoreCase(dbCurStatus)) {
						getHttpServletRequest().setAttribute(errorMessage,
								"<font color=\"red\" size=\"1.5\">Sorry! Your account will be activated soon!</font>");
						resultType = INPUT;
					} else {
						if (encPwd.equalsIgnoreCase(dbPassword.trim())) {
							if ("Active".equalsIgnoreCase(dbCurStatus)) {
								...
								session.setAttribute(ApplicationConstants.Img_Alt, dbImageAlt);
								if (dbProfilImagePath != null) {
									...
									System.out.println("---****---" + dbImgFlag);
									if (imageFile.exists() == false) {
										if ("F".equals(dbgender)) {
											session.setAttribute(ApplicationConstants.USER_IMAGE_PATH,
													Properties.getProperty("Profile.FEMALEIMAGE"));

										} else {
											session.setAttribute(ApplicationConstants.USER_IMAGE_PATH,
													Properties.getProperty("Profile.GENERALIMAGE"));
										}
									} else {
								...
								session.setAttribute(ApplicationConstants.USER_IMAGE_FLAG, dbImgFlag);
								...
								String primaryRoledetails = DataSourceDataProvider.getInstance()
										.getUsrPrimaryRole(dbUserId);
								...
								session.setAttribute(ApplicationConstants.ROLESMAP,
										DataSourceDataProvider.getInstance().getUsrRolesMap(dbUserId));
								session.setAttribute(ApplicationConstants.SKILLS_MAP,
										DataSourceDataProvider.getInstance().getSkillsCategory(1));
								...
								if (usrGrpId > 0) {
									session.setAttribute(ApplicationConstants.USER_CATEGORY_LIST,
											DataSourceDataProvider.getInstance().getUserSubCategoryByUsrId(dbUserId));
								}
								...
								if (resactionName.equals("../general/logout.action")) {
									getHttpServletRequest().setAttribute(errorMessage,
											"<font color=\"red\" size=\"1.5\">Sorry Your Account is InActive! </font>");
									resultType = INPUT;
								}
								...
								if (!resactionName.equals("")) {
									...
									setRedirectAction(
											"../general/regerrorDirect.action?resultMessage=" + getResultMessage());
								}
					...
					getHttpServletRequest().setAttribute(errorMessage,
							"<font color=\"red\" size=\"1.5\">Oraganization record not existed,please contact Admin team!</font>");
					resultType = INPUT;
				}
	 ...
	 *
	 * ForUse : consultantLogin() method is used for Consultant Login.
	 *
	 * *****************************************************************************
	 */
	public String consultantLogin() throws Exception {
		System.out.println("********************LoginAction :: consultantLogin Action Start*********************");
		...
		try {
			...
			// String SQL_QUERY = "SELECT u.usr_id as
			// usrId,login_id,salt,PASSWORD,type_of_user,first_name,last_name,"
			// +
			// "dob,cur_status,living_country,working_country,org_id,email1,image_path,phone1,marital_status,alias_name,gender,acc.status
			// FROM users u left outer join usr_reg ur on(u.usr_id=ur.usr_id) "
			// + " LEFT OUTER JOIN accounts acc ON (org_id=acc.account_id) WHERE
			// acc.STATUS='Active' and login_id like '" +
			// getEmailId().toLowerCase().trim() + "'";
			//
			String typeOfConsultant = com.mss.msp.util.DataSourceDataProvider.getInstance()
					.getTypeOfConsultant(getEmailId().toLowerCase().trim());

			if (typeOfConsultant.equals("IC")) {
				SQL_QUERY = "SELECT u.usr_id as usrId,login_id,salt,PASSWORD,type_of_user,first_name,last_name,CONCAT(LEFT(first_name , 1),LEFT(last_name , 1)) AS ImgAlt,"
						+ "dob,cur_status,living_country,working_country,created_by_org_id,email1,image_path,phone1,marital_status,alias_name,gender,acc.status FROM users u left outer join usr_reg ur on(u.usr_id=ur.usr_id) "
						+ " LEFT OUTER JOIN accounts acc ON (created_by_org_id=acc.account_id) WHERE acc.STATUS='Active' and login_id like '"
						+ getEmailId().toLowerCase().trim() + "'";
			} else {

				SQL_QUERY = "SELECT u.usr_id AS usrId,login_id,salt,PASSWORD,type_of_user,first_name,last_name,dob,cur_status,CONCAT(LEFT(first_name , 1),LEFT(last_name , 1)) AS ImgAlt,"
						+ " living_country,working_country,email1,image_path,phone1,marital_status,alias_name,gender FROM "
						+ " users u LEFT OUTER JOIN " + " usr_reg ur ON(u.usr_id=ur.usr_id) "
						+ "  WHERE  login_id like '" + getEmailId().toLowerCase().trim() + "'";
			}
			...
			while (resultSet.next()) {
				...
				dbCurStatus = resultSet.getString("cur_status");
				System.out.println("dbCurStatus==>" + dbCurStatus);
				if (resultSet.getString("living_country") != null) {
					dbLivingCountry = com.mss.msp.util.DataSourceDataProvider.getInstance()
							.getCountry(resultSet.getInt("living_country"));
				}
			...
			if ("Registered".equalsIgnoreCase(dbCurStatus)) {
				getHttpServletRequest().setAttribute("consultantErrorMessage",
						"<font color=\"red\" size=\"1.5\">Sorry! Your account will be activated soon!</font>");
				resultType = INPUT;
			} else {
				if (encPwd.equalsIgnoreCase(dbPassword.trim())) {
					if ("Active".equalsIgnoreCase(dbCurStatus)) {
						...
						System.out.println("---*******dbImgFlag********-----" + dbImgFlag);
						session.setAttribute(ApplicationConstants.MSTATUS, dbmstatus);
						session.setAttribute(ApplicationConstants.AliasName, dbalias);
						session.setAttribute(ApplicationConstants.GENDER, dbgender);
						session.setAttribute(ApplicationConstants.DOB, dbDOB);
						...
						session.setAttribute(ApplicationConstants.SKILLS_MAP,
								DataSourceDataProvider.getInstance().getReqSkillsCategory(1));

						System.out.println("In login action" + ApplicationConstants.TYPE_OF_USER);
						System.out.println("In login action Skills" + ApplicationConstants.SKILLS_MAP);

						String resactionName = "/recruitment/consultantLogin/getConsultantProfile.action";
						/*
						 * resactionName =
						 * SecurityServiceProvider.doRedirect(dbOrgId,
						 * dbTypeOfUsr, primaryRole); if
						 * (resactionName.equals("../general/logout.action")) {
						 * getHttpServletRequest().setAttribute("errorMessage",
						 * "<font color=\"red\" size=\"1.5\">Sorry Your Account is InActive! </font>"
						 * ); resultType = INPUT; } if
						 * (resactionName.equals("")) { resactionName =
						 * SecurityServiceProvider.doRedirect(0, dbTypeOfUsr,
						 * primaryRole); }
						 * 
						 * System.err.println("resactionName==>" +
						 * resactionName); if (!resactionName.equals("")) {
						 * setRedirectAction(resactionName); } else {
						 * setResultMessage("Please contact Support team!");
						 * setRedirectAction(
						 * "../general/regerrorDirect.action?resultMessage=" +
						 * getResultMessage()); }
						 */
						setRedirectAction(resactionName);
			...
			getHttpServletRequest().getSession(false).setAttribute("consultantErrorMessage", ex.toString());
		...
		System.out.println("********************LoginAction :: consultantLogin() method End*********************");
		return resultType;
	}
	...
	public String accessDeneid() throws Exception {
		...
		getHttpServletRequest().setAttribute(errorMessage,
				"<font color=\"red\" size=\"1.5\">Access is denied!. Please Login. </font>");
		...
		try {
			if (getHttpServletRequest().getSession(false) != null) {
				if (getHttpServletRequest().getSession(false).getAttribute(ApplicationConstants.LOGIN_ID) != null) {
					String UserId = getHttpServletRequest().getSession(false)
							.getAttribute(ApplicationConstants.LOGIN_ID).toString();
	...
	public int getCustomerOrVenFlag() {
		return customerOrVenFlag;
	}

	public void setCustomerOrVenFlag(int customerOrVenFlag) {
		this.customerOrVenFlag = customerOrVenFlag;
	}
