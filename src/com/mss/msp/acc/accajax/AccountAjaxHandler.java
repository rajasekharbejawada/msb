...
public class AccountAjaxHandler extends ActionSupport implements ServletRequestAware, ServletResponseAware {
    ...
    private String accountLastAccessDate;
...
//    }
    ...
    public String getSearchAccountsList() {
        resultMessage = LOGIN;
        System.out.println("********************AccountAjaxHandler :: getSearchAccountsList Method Start*********************");
        if (servletRequest.getSession(false).getAttribute(ApplicationConstants.USER_ID) != null) {
            try {
            	Integer orgId = Integer.parseInt(servletRequest.getSession(false).getAttribute(ApplicationConstants.ORG_ID).toString());
                //session usr id and primary role check wether csr or not if csr send usr_id
                int usrPriRole = Integer.parseInt(servletRequest.getSession(false).getAttribute(ApplicationConstants.PRIMARYROLE).toString());
                

                int csrId = 0;
                if (usrPriRole == 1) {
                    csrId = Integer.parseInt(servletRequest.getSession(false).getAttribute(ApplicationConstants.USER_ID).toString());
                }

            	String list = ServiceLocator.getAccountAjaxHandlerService().getSearchAccountsList(servletRequest,this,orgId,csrId);
                servletResponse.setHeader("Cache-Control", "no-cache, no-store, must-revalidate");
                servletResponse.setHeader("Pragma", "no-cache");
                servletResponse.setDateHeader("Expires", 0);
                servletResponse.setContentType("text");
                servletResponse.setCharacterEncoding("UTF-8");
                servletResponse.getWriter().write(list);
                resultMessage = null;
            } catch (Exception ex) {
                ex.printStackTrace();
                resultMessage = ERROR;
        ...
        System.out.println("********************AccountAjaxHandler :: getSearchAccountsList Method End*********************");
        return null;
    }
	...
	public void setAccountLastAccessDate(String accountLastAccessDate) {
