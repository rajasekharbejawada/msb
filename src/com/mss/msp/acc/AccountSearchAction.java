...
public class AccountSearchAction extends ActionSupport implements ServletRequestAware {
    ...
    Map<String,String> staffingMode=new HashMap<String, String>();
    /**
     * Page Search Results *
     */
...
/*                Integer orgId = Integer.parseInt(httpServletRequest.getSession(false).getAttribute(ApplicationConstants.ORG_ID).toString());
    ...
    private void populateData() {
        ...
        if(getAccount().getStaffingModeTemp()!=null && !"".equals(getAccount().getStaffingModeTemp()))
        {
        	StringTokenizer st=new StringTokenizer(getAccount().getStaffingModeTemp(),",");
        		while(st.hasMoreTokens())
        		{
                String s=st.nextToken();
                staffingMode.put(s,s);
        	}
        	getAccount().setStaffingModeList(staffingMode);
        }
        else
        {
        	//staffingMode=Collections.<String, String>emptyMap();
        	getAccount().setStaffingModeList(staffingMode);
        }
