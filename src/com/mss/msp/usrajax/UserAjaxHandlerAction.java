...
import com.mss.msp.util.VerifyRecaptcha;
...
//    private int customerId;
    ...
    private String consultantEmailCheck;
    
    private String recaptchaResponse;
    private String other_skill;
    private String skill_type;
    private String action_select;
    
    public String getAction_select() {
		return action_select;
	}

	public void setAction_select(String action_select) {
...
// Use relative path for Unix systems
     ...
     *
     * ForUse : approveOtherSkills()
     * 
     *
     * *****************************************************************************
     */
    public String approveOtherSkills() {
        resultType = LOGIN;
         System.out.println("********************UsersdataHandlerAction :: approveOtherSkills Method Start*********************");
         if (httpServletRequest.getSession(false).getAttribute(ApplicationConstants.USER_ID) != null) {
            try {
            	String resultMessage = "";
                setUserSessionId(Integer.parseInt(httpServletRequest.getSession(false).getAttribute(ApplicationConstants.USER_ID).toString()));
                if(getAction_select().equalsIgnoreCase("Approve")){
                 resultMessage = DataSourceDataProvider.getInstance().skillCheck(getEmpId(), getOther_skill(), getSkill_type(), getAction_select());
                System.out.println("result-------"+resultMessage);
                }
                if(resultMessage.equals("Not Existed")|| resultMessage == ""){
                 resultMessage = ServiceLocator.getUserAjaxHandlerService().approveOtherSkills(this, userSessionId, getEmpId(), getOther_skill(), getSkill_type(), getAction_select() );
                System.out.println("Result message after skill approval--->"+resultMessage);
                }
                httpServletResponse.setHeader("Cache-Control", "no-cache, no-store, must-revalidate");
                httpServletResponse.setHeader("Pragma", "no-cache");
                httpServletResponse.setDateHeader("Expires", 0);
...
public String consultantEmailIdCheck() {
      ...
      //  if (httpServletRequest.getSession(false).getAttribute(ApplicationConstants.ORG_ID) != null) {
            try {
                result = DataSourceDataProvider.getInstance().consultantEmailIdCheck(getConsultantEmailCheck());
                ...
                if (result == 0) {
                	httpServletResponse.setContentType("text");
                	httpServletResponse.setCharacterEncoding("UTF-8");
                	httpServletResponse.getWriter().write("notExist");
                } else {
                	httpServletResponse.setContentType("text");
                	httpServletResponse.setCharacterEncoding("UTF-8");
...
public String captchaValidation() {
    ...
    System.out.println("-------In  captchaValidation class-------");
    resultMessage = LOGIN;
    boolean result;
  //  if (httpServletRequest.getSession(false).getAttribute(ApplicationConstants.ORG_ID) != null) {
        try {
            result = VerifyRecaptcha.verify(getRecaptchaResponse());
              System.out.println("result-------"+result);
            httpServletResponse.setHeader("Cache-Control", "no-cache, no-store, must-revalidate");
            httpServletResponse.setHeader("Pragma", "no-cache");
            httpServletResponse.setDateHeader("Expires", 0);      
   ...
   // }
	...
	public String getConsultantEmailCheck() {
		return consultantEmailCheck;
	}

	public void setConsultantEmailCheck(String consultantEmailCheck) {
		this.consultantEmailCheck = consultantEmailCheck;
	}
