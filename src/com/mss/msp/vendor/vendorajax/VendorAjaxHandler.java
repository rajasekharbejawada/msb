...
public class VendorAjaxHandler extends ActionSupport implements ServletRequestAware, ServletResponseAware {
    ...
    public String getAllCounts() {
        System.out.println("********************VendorAjaxHandler Action :: getAllCounts Method Start*********************");
        resultType = LOGIN;
        String result = "";
        try {
            if (httpServletRequest.getSession(false).getAttribute(ApplicationConstants.USER_ID) != null) {
               setSessionOrgId(Integer.parseInt(httpServletRequest.getSession(false).getAttribute(ApplicationConstants.ORG_ID).toString()));
                setUserSessionId(Integer.parseInt(httpServletRequest.getSession(false).getAttribute(ApplicationConstants.USER_ID).toString()));
                result = ServiceLocator.getVendorAjaxHandlerService().getAllCounts(this);
                httpServletResponse.setHeader("Cache-Control", "no-cache, no-store, must-revalidate");
                httpServletResponse.setHeader("Pragma", "no-cache");
                httpServletResponse.setDateHeader("Expires", 0);
                httpServletResponse.setContentType("text");
        ...
        System.out.println("********************VendorAjaxHandler Action :: getAllCounts Method End*********************");
        return null;
    }
