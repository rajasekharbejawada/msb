													...
													<div class="row">
														<div class="col-sm-3">
															...
															<s:select cssClass="SelectBoxStyles form-control"
																name="interviewType" id="interviewType"
														...
														<div class="col-sm-2 pull-right">
															...
															<s:submit type="button"
																...
																onclick="return getSearchTechReviewList()"
																cssStyle="margin:5px">
														...
														<table id="techReviewTable"
															...
															<tbody>
																<tr>
																	...
																	<s:if
																		test="#session.primaryrole == 3||#session.primaryrole == 13||#session.primaryrole == 1">
																		<th>Forwarded By</th>
																	</s:if>
																	...
																	<s:if
																		test="#session.primaryrole == 3||#session.primaryrole == 13||#session.primaryrole == 1 || #session.primaryrole == 11">
																		<th>Notification</th>
																	</s:if>
																</tr>
																...
																<s:iterator value="consultantList">
																	...
																	<s:url id="myUrl"
																		action="../../recruitment/consultantLogin/getConsultantProfile.action?consultFlag=customer&accountFlag=%{#session.primaryrole}&customerFlag=customer">
																		<s:param name="consult_id" value="%{consult_id}" />
																		<s:param name="vendorcomments"
																			value="%{vendorcomments}" />
																		<s:param name="requirementId" value="%{requirementId}" />
																		<s:param name="pageFlag" value ="%{pageFlag}"/>
 																	</s:url>
																	<s:url id="reqURL"
																		action="../../Requirements/requirementedit.action?vendor=yes&accountFlag=MyRequirements">
																		<s:param name="jdId" value="%{jdId}" />
																	...
																	<s:url id="examURL"
																		action="../../onlineExam/Ques/onlineexam.action">
																		<s:param name="token" value="%{consultantExamToken}" />
																		<%--<s:param name="accountSearchID" value="%{accountSearchID}"/>
                                                                            <s:param name="requirementId" value="%{requirementId}"/> --%>
																	</s:url>
																	<tr>

																		<s:if test="#session.primaryrole == 16">
																			<td><s:property value="%{jdId}"></s:property></td>
																			<td><s:property value="%{consult_name}"></s:property></td>
																		</s:if>
																		<s:else>
																			<td><s:a href='%{#reqURL}'>
																					<s:property value="%{jdId}"></s:property>
																				</s:a></td>
																			<td><s:a href='%{#myUrl}'>
																		...
																		<s:if
																			test="reviewType=='Online' || reviewType=='Psychometric'">
																			<td><s:property value="forwardedDate"></s:property></td>
																		</s:if>
																		<s:else>
																			<td><s:property value="scheduledDate"></s:property>&nbsp;&nbsp;<s:property
																					value="scheduledTime"></s:property>&nbsp;&nbsp;<s:property
																					value="zone"></s:property></td>
																		</s:else>
																		<s:if
																			test="#session.primaryrole == 3 || #session.primaryrole == 13||#session.primaryrole == 1">
																			<td><s:a href="#"
																					cssClass="Forwarded_popup_open"
																					onclick="showOverlayForwardedBy(%{forwardedById})">
																					<s:property value="forwardedBy"></s:property>
																				</s:a></td>
																		</s:if>
																		<s:elseif test="#session.primaryrole == 11">
																		...
																		<s:if test="#session.primaryrole == 1">
																			<td><s:property value="customerName"></s:property></td>
																			<td><s:property value="vendorName"></s:property>
																			</td>
																		</s:if>
																			...
																			<td><center>
																					<figcaption>
																						<button style="cursor: default" type=button
																							value="<s:property value="resumeId"></s:property>">
																							<i class="fa fa-download" style="opacity: 0.3;"></i>
																						</button>
																					</figcaption>
																				</center></td>
																			<td><s:a href="#" cssClass=""
																					style="pointer-events: none; cursor: default; opacity: 0.5">Click</s:a></td>
																		...
																		<s:else>
																			<td><center>
																					<figcaption>
																						<button type=button
																							value="<s:property value="resumeId"></s:property>"
																							onclick=doTechReviewAttachmentDownload(<s:property value="resumeId"></s:property>)>
																			...
																			<s:else>
																				<td><s:a href="#"
																						cssClass="techReview_popup_open"
																						onclick="techReviewOverlay(%{requirementId},\'%{status}\');techReviewDetailsOnOverlay(%{consult_id},%{conTechReviewId},\'%{reviewType}\');">Click</s:a></td>
																			</s:else>
																			<s:if test="#session.primaryrole == 16">
																				<s:if
																					test="reviewType=='Online' || reviewType=='Psychometric'">
																					<td><s:a href='%{#examURL}' target="_blank">Click Here</s:a>
																					</td>
																				</s:if>
																				<s:else>
																					<td>--</td>
																		...
																		<s:if
																			test="#session.primaryrole == 3||#session.primaryrole == 13||#session.primaryrole == 1">
																			<s:if test="customerNotify==false">
																				<td><button style="cursor: default"
																						type="button"
																						onclick="sendNotificationToVendor();">Send</button></td>
																			</s:if>
																			<s:else>
																				<td><button style="cursor: default"
																						type="button" disabled>Send</button></td>
																			</s:else>
																		</s:if>
																		<s:elseif test="#session.primaryrole == 11">
																			<s:if test="vendorNotify==false">
																				<td><button style="cursor: default"
																						type="button"
																						onclick="sendNotificationToConsultant();">Send</button></td>
																			</s:if>
																			<s:else>
																				<td><button style="cursor: default"
																						type="button" disabled>Send</button></td>
											...
											<span id="online"> <s:hidden id="contechId"
												...
												<div class="pull-left ">
													<s:submit type="button" cssClass="cssbutton fa fa-times"
														value="Rejected"
														onclick="saveExamResult(this.value,'reviewDetails');"
