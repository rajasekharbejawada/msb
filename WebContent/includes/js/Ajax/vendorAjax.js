        ...
        req.onreadystatechange = function() {
            if (req.readyState == 4 && req.status == 200) {
                if (req.responseText > 0)
                    $("saveVendorAssociation").html(" <font color='green'> " + req.responseText + " Vendors associated successfully</font>");
                else
...
function validateVendorAssociation() {
    ...
    if (accessTime == "") {
        //  alert("desc");
        $("saveVendorAssociation").html("<font color='red'>Please select access time</font>");
    ...
    if (response.length > 0) {
        for (var i = 0; i < eduList.length - 1; i++) {
            ...
            var Values = eduList[i].split("|");
            {
                ...
                //row.append($("<td>" + Values[7] + "</td>"));
                //onclick="saveContactDetails(' + Values[0] +');" > '
                row.append($('<td><a href=.././Requirements/vendorRatingAndReveiw.action?vendorOrgId='+Values[9]+'&accountName='+Values[5].replace(/\s/g, '%20') +'>Click</td>'));
            }
        }
    }
    else {
...
function getConsultantListBySearch() {
    ...
    var consult_ssnNo = $("input[name=consult_ssnNo]").val();
    consult_ssnNo=consult_ssnNo.replace(/-/g, "");
    ...
    var url = 'Requirements/getConsultantListBySearch?RequirementId=' + RequirementId + '&consult_name=' + consult_name +
            '&consult_email=' + consult_email + '&consult_skill=' + consult_skill + '&consult_phno=' + consult_phno + '&vendor=' + vendorName + '&consult_lstname=' + consult_lstname + '&consult_ssnNo=' + consult_ssnNo +'&yearExperience='+yearExperience;
    var req = initRequest(url);
    req.onreadystatechange = function() {
...
//	var pageFlag = $("#pageFlag").val();
    ...
    var pageFlag="profileForVendorCustomer";
...
//                
                ...
                if (vendor != 'yes') {
                	 row.append($('<td><a   href=../recruitment/consultantLogin/getConsultantProfile.action?pageFlag='+pageFlag+'&vendor=' + vendor + '&consult_id=' + Values[0] + '&consultFlag=' + consultFlag + '&requirementId=' + requirementId + '&accountSearchID=' + accountSearchID + '&accountFlag=' + roleId + '&jdId=' + jdId + '&customerFlag=' + customerFlag + '&consult_salary=' + Values[8] + '&vendorcomments=' + v_comments + '&consultStatus=' + Values[5] + '&consultantAppStatus='+ Values[5]+">" + Values[1] + "</a></td>"));

                } else {
                    row.append($('<td><a href=../recruitment/consultantLogin/getConsultantProfile.action?vendor=' + vendor + '&consult_id=' + Values[0] + '&consultFlag=' + consultFlag + '&requirementId=' + requirementId + '&accountSearchID=' + accountSearchID + '&accountFlag=' + roleId + '&jdId=' + jdId + '&customerFlag=' + customerFlag + '&consult_salary=' + Values[8] + '&pageFlag=' + pageFlag + '&vendorcomments=' + v_comments + ">" + Values[1] + "</a></td>"));
                }
                //row.append($("<td>" + Values[7] + "</td>"));
...
{//{  alert("hi")
    ...
    var recordPage=document.getElementById("accpaginationOption").value;
    //alert(accountFlag)
    if (reqStart != "")
    {
        if (reqEnd == "")
    ...
    var url = 'getSearchRequirementsList.action?jobTitle=' + jobTitle + '&requirementSkill=' + requirementSkill + '&requirementStatus=' + requirementStatus + '&reqStart=' + reqStart + '&reqEnd=' + reqEnd + '&accountFlag=' + accountFlag + '&vendor=' + vendor + '&accountSearchID=' + accountSearchID + '&reqCategoryValue=' + reqCategory+'&pgFlag='+pGflag+'&pgNo='+pgNo+'&recordPage='+recordPage;
    //alert(url);
...
//      alert(orgid);
    ...
    if(flag=='1'){
        //alert(dataArray[1])
          ...
          document.getElementById("reqInAccTotalRecords").value=dataArray[1];
      }
    if (response.length > 0) {
        for (var i = 0; i < reqList.length - 1; i++) {
            ...
            var Values = reqList[i].split("|");
            {
                ...
                if(Values[4]=='Opened'){
                reqRow.append($('<td><center><a href="#" class="releaseVendorOverlay_popup_open" onclick="selectVendorsOverlayForCSR('+Values[0]+','+orgid+',\''+Values[14]+ '\');">'+"<i class='fa fa-arrow-circle-o-right fa-size'></i></center></td>"));
                }
                else{
...
function selectVendorsOverlayForCSRClose(){
...
function selectVendorsOverlayForCSR(requirementId,orgId,taxTerm){ 
...
//  document.getElementById('releaseVendorDiv').innerHTML = '';
  ...
  $('#releaseVendorOverlay_popup').popup(); 
  var url='./getVendorsBasedOnReqCategoryForCSR.action?requirementId='+requirementId+'&orgId='+orgId+'&taxTerm='+taxTerm;
 var req=initRequest(url);
  req.onreadystatechange = function() {
      if (req.readyState == 4) {
          if (req.status == 200) {
              ...
              populateReleaseVendorsOverlayForCSR(req.responseText);
          } 
      }
  };
  req.open("GET",url,"true");
...
function populateReleaseVendorsOverlayForCSR(data){
	 var vendorsIdString='';
	    var flag=data.split("FLAG");
	    var addList=flag[0].split("^^");
	    ...
	    var row = $("<tr/>")
	    $("#releaseReqTable").append(row); //this will append tr element to table... keep its reference for a while since we will add cels into it
//	    if(taxTerm=='CO')
...
//	    	row.append($("<th>").append("HeadHunters Vendors List"));
	    if (flag.length > 0) {
	        ...
	        //                 row.append($("<td>" + Values[1] + "</td>"));///alert("oooooooooooooooo"+$(label))
	        }
	    }
	    else {
	        var row = $("<tr />")
...
function onClickHandlerForCSR(res){
	var check =res.checked;
//	alert(check)
	if (check ==true){
...
function doReleaseRequirementForCSR(){
	var count =0;
    var vendorsIdString = document.getElementById("vendorsIdString").value;
    ...
    var url='./doReleaseRequirementsForCSR.action?requirementId='+requirementId+
    '&orgId='+orgId+'&taxTerm='+taxTerm+'&vendorOrgIds='+vendorOrgIds;//alert(url)
    var req=initRequest(url);
    req.onreadystatechange = function() {
        if (req.readyState == 4 && req.status == 200) {
            if(req.responseText==0){
...
/*
function getReleaseVendorsForCsr(requirementId,orgId,taxTerm){
	 window.location= "../acc/getReleaseVendors.action?requirementId="+requirementId+'&orgId='+orgId+'&taxTerm='+taxTerm;
}



function getReleaseVendorsListForCsr(){ 
	document.getElementById("vendorReleaseButton").style.display = "block";
//  document.getElementById('releaseVendorDiv').innerHTML = '';
 var requirementId= document.getElementById("requirementId").value;
 var orgId= document.getElementById("orgId").value;
 ...
 var tier= document.getElementById("tier").value;
  ...
  var url='./getVendorsBasedOnReqCategoryForCSR.action?requirementId='+requirementId+'&orgId='+orgId+'&taxTerm='+taxTerm+'&vendor='+vendorName+'&vendorTierType='+tier;
 var req=initRequest(url);
  req.onreadystatechange = function() {
      if (req.readyState == 4) {
          if (req.status == 200) {
              ...
              document.getElementById("orgId").value = orgId;
              document.getElementById("taxTerm").value = taxTerm;
              populateReleaseVendorsListForCSR(req.responseText);
...
function populateReleaseVendorsListForCSR(data){
	 var vendorsIdString='';
	    var flag=data.split("FLAG");
	    var addList=flag[0].split("^^");
	     var table = document.getElementById("releaseReqTable");
	    ...
	    $("#releaseReqTable").append(row); //this will append tr element to table... keep its reference for a while since we will add cels into it
          if (flag.length > 0) {
	        ...
	        for (var i = 0; i < addList.length-1; i++) {
	            ...
	            cell = document.createElement("TD");
	            cell.appendChild(document.createTextNode(Values[1]));
	            row.appendChild(cell);
	            
	            cell = document.createElement("TD");
	            if(Values[2]==1){
	            	cell.innerHTML = "Tier-1";
	    	          
	            }else  if(Values[2]==2){
	            	cell.innerHTML = "Tier-2";
	    	          
	            }else  if(Values[2]==3){
	            	cell.innerHTML = "Tier-3";
	            ...
	            checkbox.value = 0;
	            checkbox.id = "vendor"+Values[0];
	            row.appendChild(cell).append(checkbox);
...
//	alert(check)
	if (check ==true){
		 var vendorsIdString = document.getElementById("vendorsIdString").value;
		 alert("vendorsIdString.."+vendorsIdString)
		 var vendorIds=vendorsIdString.split("^^");
		    var vendorOrgIds='';
		    for(var i =0;i<vendorIds.length-1;i++){
		    	document.getElementById("vendor"+vendorIds[i]).checked=false;
...
function doReleaseRequirementForCSR(){
    ...
    alert("vendorOrgIds..."+vendorOrgIds);
    if(count==0){
        $("releaseMessage").html(" <font color='red'>Please select atleast one vendor</font>");
        $("#reqResultMsg").fadeOut(5000);
        return false;
...
function showChart(month, reqCount, selected, rejected, servicing, shortlisted, processing)
{
    ...
    var children = processing.concat(servicing).concat(selected).concat(shortlisted).concat(rejected); 
    var v=Math.max.apply(Math, children);
...
function getSearchAccList(element)
{
	var pgNo=document.getElementById("pgNo").value;
	//alert("PGNO-->"+pgNo);
    var pGflag=element;
   ...
   var accountLastAccessDate=$("#accountLastAccessDate").val();
   var accCountry=$("#accCountry").val();
   var accountState=$("#accountState").val();
   var accountType=$("#accountType").val();
   var accountIndustry=$("#accountIndustry").val();
   var accountStatus=$("#accountStatus").val();
   var diversity=$("#diversity").val();
   //var staffingModeTemp=$("#staffingModeTemp").val();
   var recordPage=$("#paginationOption").val();
   var staffingModearray = [];
   $("#staffingModeList :selected").each(function() {
   	staffingModearray.push($(this).text());
   }); 
    var url = CONTENXT_PATH+'/acc/getSearchAccountsList.action?accountName=' + accountName + '&accountUrl=' + accountUrl + '&accountZip=' + accountZip + '&accountLastAccessDate=' + accountLastAccessDate + '&accCountry=' + accCountry + '&accountType=' + accountType + '&accountIndustry=' + accountIndustry + '&accountStatus=' + accountStatus + '&diversity=' + diversity+ '&staffingMode=' + staffingModearray+'&pgFlag='+pGflag+'&pgNo='+pgNo+'&recordPage='+recordPage+'&accState='+accountState;
    //alert(url);
    ...
    req.onreadystatechange = function() {
        ...
        if (req.readyState == 4 && req.status == 200) {
            $('#loadingAcoountSearch').hide();
            // alert("response"+req.responseText);
            populateAccTableReq(req.responseText,element);
        }
    };
    req.open("GET", url, "true");
    req.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
...
function populateAccTableReq(response,flag) {
   // alert(response);  
    $(".page_option").css('display', 'block');
    var dataArray = response.split("###");
    var accList = dataArray[0].split("^");
    var table = document.getElementById("accountSearchResults");
    var recordPage=document.getElementById("paginationOption").value;
    var accFlag="accDetails";
    var link;
    for (var i = table.rows.length - 1; i > 0; i--)
    {
        table.deleteRow(i);
    ...
    if(flag=='1'){
      //  alert("total records are "+dataArray[1]);
          document.getElementById("pgNo").value=1;
          document.getElementById("searchAccountTotalRecords").value=dataArray[1];
      }
    var urlLink,aLink;
    if ((document.getElementById("searchAccountTotalRecords").value) > 0) {
        for (var i = 0; i < accList.length - 1; i++) {

            var Values = accList[i].split("|");
            {
            	
                var reqRow = $("<tr />")
                $("#accountSearchResults").append(reqRow); //this will append tr element to table... keep its reference for a while since we will add cels into it
                	reqRow.append($(("<td>" + Values[0] + "</td>")));
                	
                    //reqRow.append($('<td><a href="../acc/viewAccount.action?accountSearchID=' + Values[0] + '&accountFlag='+accFlag +">"+Values[1]+"</a></td>"));
                    //reqRow.append($("<td><a href='" + Values[1] +"'></a></td>"));
                	aLink="../acc/viewAccount.action?accountSearchID=" + Values[1] + "&accFlag=accDetails";
                	reqRow.append($("<td><a href="+aLink+"> " + Values[2] + "</a></td>"));
                			
                	urlLink="http://"+Values[3];
                    reqRow.append($("<td><a href="+urlLink+" target='_blank'>" + Values[3] + "</a></td>"));	
                    reqRow.append($("<td>" + Values[4] + "</td>"));
                    reqRow.append($("<td>" + Values[5] + "</td>"));
                    reqRow.append($("<td>" + Values[6] + "</td>"));
                    reqRow.append($("<td>" + Values[7] + "</td>"));
                    reqRow.append($("<td>" + Values[8] + "</td>"));
                    reqRow.append($("<td>" + Values[9] + "</td>"));
                    reqRow.append($("<td>" + Values[10] + "</td>"));
             }
        }
       // $('#accountSearchResults').tablesPaginate({navigateType:'navigator'},recordPage,accountSearchResults);
        ...
        var row = $("<tr />")
        $("#accountSearchResults").append(row);
        row.append($('<td colspan="9"><font style="color: red;font-size: 15px;">No Records to display</font></td>'));
        $(".page_option").css('display', 'none');
    }
