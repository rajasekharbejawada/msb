...
function getCustomerDashboardList(){
    ...
    document.getElementById('chartTitle').innerHTML = "<font color=green>Customer Requirements Monthly Analysis</font>";
...
function showReqPieChart(response){
  //  document.getElementById("QReviewDashBoardByStatusPie").style.display="block";
	 var dashBoardReq=response.split("^");
	
	  
//	  alert(response+"in showing pie chart method")
	
	
	  var arraydata1 = [['Total', 'Count']];
	var result1=['Open','Release','Open For Resume','Close','Others'];
	var openTotal=0,releaseTotal=0,ORTotal=0,closeTotal=0,otherTotal=0;
	for (var i = 0; i < dashBoardReq.length - 1; i++) {
		
	 var res =dashBoardReq[i].split("|");
	 var values=[res[1],res[2],res[6],res[3],res[5]]
	
	 for(var j=0; j<result1[1].length-2; j++){
//		 alert("result1[j].."+result1[j]+"values[j]..."+values[j]);
		
		    if(result1[j]=='Open')
		    	openTotal = openTotal + parseInt(values[j]);
		    else if(result1[j]=='Release')
		    	releaseTotal = releaseTotal +parseInt(values[j]);
		    else if(result1[j]=='Open For Resume')
		    	ORTotal = ORTotal + parseInt(values[j]);
		    else if(result1[j]=='Close')
		    	closeTotal =closeTotal + parseInt(values[j]);
		    if(result1[j]=='Others')
		    	otherTotal = otherTotal +parseInt(values[j]);
		
	    }
	
	
	}
//	var status = [openTotal,releaseTotal,ORTotal,closeTotal,otherTotal]
//	for(int k=0;k<result1[1].length-2; k++)
//		{
//		 var dArray = [result1[k],status[k]];
//		 arraydata.push(dArray);
//		}
	var total_count = openTotal+releaseTotal+ORTotal+closeTotal+otherTotal;
	 var arraydata = [['Total', total_count]];
	 var dArray = [result1[0],((openTotal/total_count)*100).toFixed(2)+"%"];
	 var pArray = [result1[0],openTotal];
	 arraydata.push(dArray);
	 arraydata1.push(pArray);
	 dArray = [result1[1],((releaseTotal/total_count)*100).toFixed(2)+"%"];
	 pArray = [result1[1],releaseTotal];
	 arraydata.push(dArray);
	 arraydata1.push(pArray);
	 dArray = [result1[2],((ORTotal/total_count)*100).toFixed(2)+"%"];
	 pArray = [result1[2],ORTotal];
	 arraydata.push(dArray);arraydata1.push(pArray);
	 dArray = [result1[3],((closeTotal/total_count)*100).toFixed(2)+"%"];
	 pArray = [result1[3],closeTotal];
	 arraydata.push(dArray);arraydata1.push(pArray);
	 dArray = [result1[4],((otherTotal/total_count)*100).toFixed(2)+"%"];
	 pArray = [result1[4],otherTotal];
     arraydata.push(dArray);arraydata1.push(pArray);
    var data = google.visualization.arrayToDataTable(arraydata1);
    var options = {
        title: '' ,
        titleTextStyle: {
...
//            alert(activityType)
            getRequirementPieChartDetails(activityType);
...
//        ];
    ...
    var height = $('#funnelPanel').height();// alert(height+"==========")
        var funnel_chart = new D3Funnel("#funnel");
        var funnel_options = {
          chart: {
            width:  300,   
            height:200
        ...
        funnel_chart.draw( funnel_data ,funnel_options);
...
function getRequirementPieChartDetails(activityType){
    ...
    if(activityType =="Open")
    	 activityType = "O";
    else if(activityType =="openForResume")
   	 activityType = "OR";
    else if(activityType =="Release")
   	 activityType = "R";
    else if(activityType =="close")
   	 activityType = "C";
//    else if(activityType =="others")
      
    
    var url='../dashboard/getCustomerRequirementPieChartDetails.action?dashYears='+dashYears+'&dashMonths='+dashMonths+'&activityType='+activityType;//alert(url)
    var req=initRequest(url);
    req.onreadystatechange = function() {
    	document.getElementById('loadingConsultant').style.display = 'block';
        if (req.readyState == 4 && req.status == 200) {
//         alert(req.responseText)
        	$('#loadingConsultant').hide();
            populateCustomerRequirementPieChartDetails(req.responseText);
...
function populateCustomerRequirementPieChartDetails(response){
	document.getElementById("pieChartDrillDownGrid").style.display ="block";
	 $(".page_option").css('display', 'block');
     $(".pagination").css('display', 'block');
	  var dashBoardReq=response.split("^");
	    var table = document.getElementById("customerPieChartDetails");
	    for(var i = table.rows.length - 1; i > 0; i--)
	    {
	        table.deleteRow(i);
	    }
	    if(response.length>0){
	        ...
	        for(var i=0;i<dashBoardReq.length-1;i++){   
	            ...
	            {  
	                var row = $("<tr />")
	                $("#customerPieChartDetails").append(row); //this will append tr element to table... keep its reference for a while since we will add cels into it
	                row.append($("<td>" + Values[0] + "</td>"));
	                row.append($("<td>" + Values[1] + "</td>"));
	                row.append($("<td>" + Values[3] + "</td>"));
	                row.append($("<td>" + Values[4] + "</td>"));
...
function showChart(cust,open,release,close)
{
    ...
    var options = {
        //        width: 370,
        //        height:300,
        title: 'Customer Requirements Monthly Analysis',
...
function showCustomerChart(month,open,release,openForResume,close,others)
{
    ...
    var options = {
        ...
        colors: ['#0000FF', '#00FF00', '#FF0000','#00BFFF','#B637A6'],
        legend: {
            position: 'top', 
            alignment: 'center'
        }
...
function getVendorReqDashBoardGrid(){
	document.getElementById("loadingRequirementsDash").style.display="block";
	var job_id=$('#job_id').val();
    var candidateName=$('#candidateName').val();
    var jobTitle=$('#jobTitle').val();
    //alert("HI "+csrCustomers+"  "+dashYears)
    
    var url='../vendor/getVendorReqDashBoardGrid.action?candidateName='+candidateName+'&jobTitle='+jobTitle+'&job_id='+job_id;
    var req=initRequest(url);
    req.onreadystatechange = function() {
        if (req.readyState == 4 && req.status == 200) {
            //alert(req.responseText)
        	document.getElementById("loadingRequirementsDash").style.display="none";
    ...
    if(response.length>0){
        ...
        for(var i=0;i<dashBoardReq.length-1;i++){   
            ...
            {  
                ...
                var reqLink="../Requirements/requirementedit.action?jdId=" + Values[0] + "&accountSearchID=" + Values[6] + "&requirementId=" + Values[5] + "&accountFlag=MyRequirements&vendor=yes";
                row.append($("<td><a href="+reqLink+">" + Values[0] + "</a></td>"));
                row.append($("<td>" + Values[1] + "</td>"));
               var actionLink="../recruitment/consultant/editConsultantDetails.action?consultFlag=vendor&consult_id=" + Values[4] + "&consultantFlag=My";
            	row.append($("<td><a href="+actionLink+"> " + Values[2] + "</a></td>"));
                row.append($("<td>" + Values[3] + "</td>"));
            }
        }
    }
...
// To get Monthly Analysis counts
 function getAllCounts(){
	 //alert("loading count");
	 var url='../vendor/getAllCounts.action';
	 //alert(url);
	    var req=initRequest(url);
	    req.onreadystatechange = function() {
	        if (req.readyState == 4 && req.status == 200) {
	        	...
	        	if(response.length>0){
	                ...
	                document.getElementById("reqCnt").innerHTML = Values[0];
	                document.getElementById("selCnt").innerHTML = Values[1];
	                document.getElementById("conCnt").innerHTML = Values[2];
	                document.getElementById("intCnt").innerHTML = Values[3];
	        	}
	        	else{
	        		document.getElementById("reqCnt").innerHTML = "0";
	                document.getElementById("selCnt").innerHTML = "0";
	                document.getElementById("conCnt").innerHTML = "0";
	                document.getElementById("intCnt").innerHTML = "0";
	        	}
	        } 
	    };
	    req.open("GET",url,"true");
 ...
 function getTechReviewanalysis(){
	 //alert("test");
	    $("#techReviewChart").css('visibility', 'visible');
	    document.getElementById('techReviewChartTitle').innerHTML = "<font color=green>Tech Reviews Analysis</font>";

	 
	    //alert("HI "+csrCustomers+"  "+dashYears)
	    
	    var url='../dashboard/getTechReviewanalysis.action';
	    var req=initRequest(url);
	    req.onreadystatechange = function() {
	        if (req.readyState == 4 && req.status == 200) {
	         //alert(req.responseText)
	            getTechReviewanalysisResponse(req.responseText);
 ...
 function getTechReviewanalysisResponse(response){
	    //alert("RESULT"+response)
//	    $(".page_option").css('display', 'block');
	    var countVal=response.split("|");
//alert(countVal[0]);
	    ...
	    function drawChart() {
	      var data = google.visualization.arrayToDataTable([
	        ...
	        ["Processing", parseInt(countVal[0]), "#00aae7"],
	        ["Short Listed", parseInt(countVal[1]), "#2468a0"],
	        ["Selected", parseInt(countVal[2]), "#00aae7"],
	        ["Rejected",parseInt(countVal[3]), "##2468a0"],
	        ["Exam Completed",parseInt(countVal[4]), "#00aae7"]
	      ]);
	      ...
	      var chart = new google.visualization.BarChart(document.getElementById("techReviewChart"));
