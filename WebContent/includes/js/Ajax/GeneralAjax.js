...
function forgotPassword(emailId, resultMessage, loadingMessage) {
...
function forgotpasswordResponsehandler(response, resultMessage, loadingMessage) {
	// respone;
...
function loadingContent(loadingMessage) {
...
function getSecReportNames() {
	...
	if (v_empName == "") {
		...
		if (v_empName.length >= 3) {
			var url = CONTENXT_PATH + "/getEmployeeDetails.action?empName="
					+ v_empName;
			// alert("url-->"+url);
			var req = initRequest(url);
			req.onreadystatechange = function() {
...
function parseCustMessages(responseXML) {
	...
	if (chk.childNodes[0].nodeValue == "true") {
		...
		for (loop = 0; loop < consultants.childNodes.length; loop++) {
			...
			appendCustomer(empName.childNodes[0].nodeValue,
...
function parseCustMessagesSecRep(responseXML) {
	...
	if (chk.childNodes[0].nodeValue == "true") {
		...
		for (loop = 0; loop < consultants.childNodes.length; loop++) {
			...
			var loginId = consultant.getElementsByTagName("EMPID")[0];
			var empName = consultant.getElementsByTagName("NAME")[0];
			appendReportingPerson1(empName.childNodes[0].nodeValue,
					loginId.childNodes[0].nodeValue);
		}
		var position;
...
function parseCustMessagesSecRep2(responseXML) {
	...
	if (chk.childNodes[0].nodeValue == "true") {
		...
		for (loop = 0; loop < consultants.childNodes.length; loop++) {
			...
			var loginId = consultant.getElementsByTagName("EMPID")[0];
			var empName = consultant.getElementsByTagName("NAME")[0];
			appendReportingPerson2(empName.childNodes[0].nodeValue,
					loginId.childNodes[0].nodeValue);
		}
		var position;
...
function appendCustomer(empName, loginId) {
	...
	linkElement.setAttribute("href", "javascript:set_cust('" + empName + "','"
			+ loginId + "')");
...
function appendReportingPerson1(empName, loginId) {
	...
	linkElement.setAttribute("href", "javascript:set_ReportingPerson1('"
			+ empName + "','" + loginId + "')");
	linkElement.appendChild(document.createTextNode(empName));
	linkElement["onclick"] = new Function("hideScrollBar()");
	nameCell.appendChild(linkElement);
...
function appendReportingPerson2(empName, loginId) {
	...
	linkElement.setAttribute("href", "javascript:set_ReportingPerson2('"
			+ empName + "','" + loginId + "')");
	linkElement.appendChild(document.createTextNode(empName));
	linkElement["onclick"] = new Function("hideScrollBar()");
	nameCell.appendChild(linkElement);
...
function parseSecondaryAssigned(responseXML) {
	...
	if (chk.childNodes[0].nodeValue == "true") {
		...
		for (loop = 0; loop < consultants.childNodes.length; loop++) {
			...
			var loginId = consultant.getElementsByTagName("EMPID")[0];
			var empName = consultant.getElementsByTagName("NAME")[0];
			appendSecondaryAssigned(empName.childNodes[0].nodeValue,
					loginId.childNodes[0].nodeValue);
		}
		var position;
...
function appendSecondaryAssigned(empName, loginId) {
	...
	linkElement.setAttribute("href", "javascript:setsecondary('" + empName
			+ "','" + loginId + "')");
	linkElement.appendChild(document.createTextNode(empName));
	linkElement["onclick"] = new Function("hideScrollBar()");
	nameCell.appendChild(linkElement);
...
function setsecondary(empName, loginId) {
	...
	if (loginId == primaryAssign) {
		// document.getElementById("UpdateTaskInfo").innerHTML="<font
		// color='red'>Primary Assign To and Secondary Assign To Should not be
		// same</>"
		$("editTask")
				.html(
						" <font color='red'>Primary Assign To and Secondary Assign To Should not be same</font>")
...
// posi = position.split(",");
// document.getElementById("menu-popup").style.left = posi[0]+"px";
// document.getElementById("menu-popup").style.top =
// (parseInt(posi[1])+20)+"px";
// document.getElementById("menu-popup").style.display = "block";
// }
// if(chk.childNodes[0].nodeValue =="false") {
// var validationMessage=document.getElementById("validationMessage");
// validationMessage.innerHTML = "<font color=red> Employee doesn't Exists
// </font>";
// //document.getElementById('validationMessage').innerHTML = "<font color=red>
// Employee doesn't Exists!</font>";
// }
// }
//
// function appendSecondaryAssigned(empName,loginId) {
...
function getEmpForTechReview() {
	...
	if (v_empName == "") {
		...
		if (v_empName.length >= 1) {
			...
			url = CONTENXT_PATH + "/getTechEmployeeDetails.action?empName="
					+ v_empName + '&requirementId=' + requirementId;
			// alert(url)
			// }
			// alert("url-->"+url);
			var req = initRequest(url);
...
function parseEmpForTechReview(responseXML) {
	...
	if (chk.childNodes[0].nodeValue == "true") {
		...
		for (loop = 0; loop < consultants.childNodes.length; loop++) {
			...
			appendEmpForTechReview(empName.childNodes[0].nodeValue,
					loginId.childNodes[0].nodeValue);
		}
		var position;
...
function appendEmpForTechReview(empName, loginId) {
	...
	linkElement.setAttribute("href", "javascript:setEmpForTechReview('"
			+ empName + "','" + loginId + "')");
	linkElement.appendChild(document.createTextNode(empName));
	linkElement["onclick"] = new Function("hideScrollBar()");
	nameCell.appendChild(linkElement);
...
function getEmpForTechReview2() {
	...
	if (v_empName == "") {
		...
		if (v_empName.length >= 3) {
			...
			//                
			url = CONTENXT_PATH
					+ "/getExternalEmployee2Details.action?empName="
					+ v_empName + '&requirementId=' + requirementId;
			// alert(url)
			// }
			// alert("url-->"+url);
...
function parseEmpForTechReview2(responseXML) {
	...
	if (chk.childNodes[0].nodeValue == "true") {
		...
		for (loop = 0; loop < consultants.childNodes.length; loop++) {
			...
			appendEmpForTechReview2(empName.childNodes[0].nodeValue,
					loginId.childNodes[0].nodeValue);
		}
		var position;
...
function appendEmpForTechReview2(empName, loginId) {
	...
	linkElement.setAttribute("href", "javascript:setEmpForTechReview2('"
			+ empName + "','" + loginId + "')");
	linkElement.appendChild(document.createTextNode(empName));
	linkElement["onclick"] = new Function("hideScrollBar()");
	nameCell.appendChild(linkElement);
	...
	if (v_empName == "") {
		...
		if (v_empName.length >= 2) {
			var url = CONTENXT_PATH
					+ "/getExternalEmployeeDetails.action?empName=" + v_empName
					+ '&resourceType=' + resourceType + '&projectID='
					+ projectID;
			// alert("url-->"+url);
			var req = initRequest(url);
			req.onreadystatechange = function() {
...
function parseTeamMemberNames(responseXML) {
	...
	if (chk.childNodes[0].nodeValue == "true") {
		...
		for (loop = 0; loop < consultants.childNodes.length; loop++) {
			...
			appendTeamMemberNames(empName.childNodes[0].nodeValue,
					loginId.childNodes[0].nodeValue);
		}
		var position;
...
function appendTeamMemberNames(empName, loginId) {
	...
	linkElement.setAttribute("href", "javascript:setTeamMemberNames('"
			+ empName + "','" + loginId + "')");
	linkElement.appendChild(document.createTextNode(empName));
	linkElement["onclick"] = new Function("hideScrollBar()");
	nameCell.appendChild(linkElement);
...
function getTeamMemberReportingPersons(loginId) {
	...
	var url = 'getTeamMemberReportingPersons.action?userId=' + loginId
			+ '&projectID=' + projectID;
...
function getCSRNames() {
	...
	if (fromCSR == "") {
		...
		if (fromCSR.length >= 3) {
			url = CONTENXT_PATH + "/getCsrNamesAutoPopulate.action?csrName="
					+ fromCSR;
			// alert(url)
			// }
			// alert("url-->"+url);
...
function parseCsrname(responseXML) {
	...
	if (chk.childNodes[0].nodeValue == "true") {
		...
		for (loop = 0; loop < consultants.childNodes.length; loop++) {
			...
			appendEmpForCsrname(empName.childNodes[0].nodeValue,
					loginId.childNodes[0].nodeValue);
		}
		var position;
...
function appendEmpForCsrname(empName, loginId) {
	...
	linkElement.setAttribute("href", "javascript:setEmpForCsrName('" + empName
			+ "','" + loginId + "')");
	linkElement.appendChild(document.createTextNode(empName));
	linkElement["onclick"] = new Function("hideScrollBar()");
	nameCell.appendChild(linkElement);
...
function getUserNameForCategory() {
	...
	if (fromCSR == "") {
		...
		if (fromCSR.length >= 3) {
			url = CONTENXT_PATH + "/getExternalEmployeeDetails.action?empName="
					+ fromCSR + "&rno=" + rno;
			// alert(url)
			// }
			// alert("url-->"+url);
...
function parseUserNameForCategory(responseXML) {
	...
	if (chk.childNodes[0].nodeValue == "true") {
		...
		for (loop = 0; loop < consultants.childNodes.length; loop++) {
			...
			appendUserNameForCategory(empName.childNodes[0].nodeValue,
					loginId.childNodes[0].nodeValue);
		}
		var position;
...
function appendUserNameForCategory(empName, loginId) {
	...
	linkElement.setAttribute("href", "javascript:setUserNameForCategory('"
			+ empName + "','" + loginId + "')");
	linkElement.appendChild(document.createTextNode(empName));
	linkElement["onclick"] = new Function("hideScrollBar()");
	nameCell.appendChild(linkElement);
...
function doUserGroupingMethod() {
	...
	if (addOrUpdate == "update") {
		if (grouperRoleId != 7 && grouperRoleId != 11) {
			$("userGrouping")
					.html(
							" <font color='red'>Grouping Can be performed for employee role only</font>.");
			return false;
		}
	}
	...
	if (allValidateGrouping(userId, usrCatType, userCatArry, usrStatus,
			usrCatType, groupingId)) {
		document.getElementById("loadingUserGrouping").style.display = "block"
		var url = '../general/doUserGroupingMethod.action?userId=' + userId
				+ '&userCatArry=' + userCatArry + '&usrStatus=' + usrStatus
				+ '&groupingId=' + groupingId + '&usrDescription='
				+ usrDescription + '&usrCatType=' + usrCatType + '&primary='
				+ primary;
		// alert(url)
		var req = initRequest(url);
		req.onreadystatechange = function() {
...
function CalculateLeangth() {
	...
	if (desc >= 250) {
		document.getElementById("usrDescription").value = document
				.getElementById("usrDescription").value.substr(0, 250);
	} else {
		$("deascOpt").html(
				"<font color='green'>Need " + (250 - desc)
						+ " More characters." + "</font>.");
	}
	if (desc == 250) {
		$("deascOpt").html(
				"<font color='red'> cannot enter more then 250 characters."
						+ "</font>.");
	}
...
function checkIsExitOrNot() {
	...
	if (userId > 0) {
		var url = '../general/checkIsExitOrNotForGrouping.action?userId='
				+ userId + '&rno=' + rno;
		// alert(url)
		var req = initRequest(url);
		req.onreadystatechange = function() {
			if (req.readyState == 4) {
				if (req.status == 200) {
					...
					if (req.responseText == "true") {
						$("userGrouping").html(
								" <font color='red'>" + "User Already Grouped!"
										+ "</font>.");
...
function allValidateGrouping(userId, usrCatType, userCatArry, usrStatus) {
	...
	if (usrName.length < 3) {
		...
		$("userGrouping")
				.html(
						"<font color='red'>" + "Please Select Proper Name."
								+ "</font>");
	...
	if (usrCatType == -1) {
		$("userGrouping").html(
				"<font color='red'>" + "Please Select Category Type."
						+ "</font>");
	...
	if (usrStatus == -1) {
		$("userGrouping").html(
				"<font color='red'>" + "Please Select Status." + "</font>");
		return flase;
	}
	if (userCatArry.length == 0) {
		$("userGrouping").html(
				"<font color='red'>" + "Please Select Categories." + "</font>");
		return flase;
	}
	return true;
}
...
function getCategoryList() {
	...
	if ($("div").is("#usrCategoryValue")) {
		...
		$(".categoryReq")
				.after(
						'<select id="usrCategoryValue" class="selectivity-result-item "   onclick="" multiple="multiple" tabindex="10" name="usrCategoryValue "><option value="-1">Select skill</option></select>');
	}
	var usrCatType = document.getElementById("usrCatType").value;
	// alert(usrCatType)
...
function getAccountNamesForHomeRedirection() {
	...
	if (v_empName == "") {
		...
		if (v_empName.length >= 1) {
			...
			url = CONTENXT_PATH
					+ "/getAccountNamesForHomeRedirection.action?empName="
					+ v_empName + '&accountType=' + accountType;
			var req = initRequest(url);
			req.onreadystatechange = function() {
				if (req.readyState == 4) {
					if (req.status == 200) {
...
function parseAccountNamesNamesForHomeRedirection(responseXML) {
	...
	if (chk.childNodes[0].nodeValue == "true") {
		...
		for (loop = 0; loop < consultants.childNodes.length; loop++) {
			...
			appendAccountNamesNamesForHomeRedirection(
					empName.childNodes[0].nodeValue,
					loginId.childNodes[0].nodeValue);
		}
		var position;
...
function appendAccountNamesNamesForHomeRedirection(empName, loginId) {
	...
	linkElement.setAttribute("href",
			"javascript:setAccountNamesNamesForHomeRedirection('" + empName
					+ "','" + loginId + "')");
	linkElement.appendChild(document.createTextNode(empName));
	linkElement["onclick"] = new Function("hideScrollBar()");
	nameCell.appendChild(linkElement);
...
// for seeting the action id,action name...from page to overlay hidden form
// fields
function setAccAuthorizationValues(actionId, actionName, actionStatus,
		actionDesc) {
	// alert(userId);
	// alert(orgId);
...
function insertOrUpdateAccAuth(flag) {
	...
	if (validateAccounts()) {
		if (isNaN(actionId)) {
			var url = 'accauth/insertOrUpdateAccAuth.action?flag=' + flag
					+ '&desc=' + addingAccAuthDesc + '&status=' + accauthStatus
					+ '&actionName=' + actionName;
		} else {
			var actionHiddenName = document.getElementById("actionHiddenName").value;

			var url = 'accauth/insertOrUpdateAccAuth.action?actionId='
					+ actionId + '&flag=' + flag + '&desc=' + addingAccAuthDesc
					+ '&status=' + accauthStatus + '&actionName=' + actionName
					+ '&actionHiddenName=' + actionHiddenName;
		}
		var req = initRequest(url);
		// alert(url);
		req.onreadystatechange = function() {
			if (req.readyState == 4) {
				...
				if (req.status == 200) {
					// document.getElementById("outputMessageOfUpdate").innerHTML="Status
					// is Updated"
					// populateAccAuthTable(req.responseText);
					// location.reload();
					window.location = "getAccAuthrization.action?resultMessage="
							+ req.responseText;
				} else {
					alert("Error occured");
				}
...
function validateAccounts() {
	...
	if (action_name == "") {
		$("#validationMessage").html(
				"<font color='red'>Please enter Action name</font>");
	...
	if (addingAccAuthDesc == "") {
		// alert("desc");
		$("#validationMessage").html(
				"<font color='red'>Please enter Description</font>");
...
function getAccountNames() {
	...
	if (accName == "") {
		...
		if (accName.length >= 3) {
			var url = '../accauth/getAccountNames.action?accType=' + accType
					+ '&accName=' + accName;
			// alert("url-->"+url);
			var req = initRequest(url);
			req.onreadystatechange = function() {
...
function parseAccountNames(responseXML) {
	...
	if (chk.childNodes[0].nodeValue == "true") {
		...
		for (loop = 0; loop < consultants.childNodes.length; loop++) {
			...
			appendAccounutNames(accName.childNodes[0].nodeValue,
					accId.childNodes[0].nodeValue);
		}
		var position;
...
function appendAccounutNames(accName, accId) {
	...
	linkElement.setAttribute("href", "javascript:setAccountNames('" + accName
			+ "','" + accId + "')");
...
function getActionResorucesSearchResults() {
	...
	var url = '../accauth/getActionResorucesSearchResults.action?actionId='
			+ action_id + '&accType=' + accType + '&status=' + status
			+ '&accName=' + accountName + '&roles=' + roles;
	// alert(url)
	var req = initRequest(url);
	req.onreadystatechange = function() {
		if (req.readyState == 4) {
	...
	if (response.length > 0) {
		for (var i = 0; i < eduList.length - 1; i++) {
			...
			var Values = eduList[i].split("|");
			{
				...
				row
						.append($("<td><a href='actionResourcesForAddOrUpdate.action?id="
								+ Values[0]
								+ "&action_id="
								+ Values[5]
								+ "&action_name="
				...
				if (Values[4].length > 9) {
					row
							.append($('<td><a href="#" class="authAccOverlay_popup_open" onclick="authResourceOverlayFun(\''
									+ Values[4]
									+ '\');">'
									+ Values[4].substring(0, 10) + "</td>"));
				} else {
				...
				if (Values[3] == "Active") {
					row
							.append($("<td><a href='#' onclick=actionResourceTermination("
									+ Values[0]
									+ ","
									+ Values[5]
									+ ",\'"
									+ Values[3]
									+ "\')><i class='fa fa-trash-o fa-size'></i></td>"));
				}
					...
					row
							.append($("<td><a href='#' onclick=actionResourceTermination("
									...
									+ ",\'"
									+ Values[3]
									+ "\')><i class='fa fa-check fa-size'></i></td>"));
				}
		...
		$("#empCategorizationResults").append(row);
		row
				.append($('<td colspan="6"><font style="color: red;font-size: 15px;">No Records to display</font></td>'));
...
function insertOrUpdateActionResources(flag) {
	...
	// var csrStatus=document.getElementById("csrStatus").value;
	var url = '../accauth/insertOrUpdateActionResources.action?blockFlag='
			+ blockFlag + '&actionId=' + actionId + '&desc=' + desc + '&flag='
			+ flag + '&id=' + id + '&accType=' + accType + '&status=' + status
			+ '&orgId=' + orgId + '&roles=' + roles + '&userGroupId='
			+ userGroupId + '&actionHiddenRole=' + actionHiddenRole;
	// alert(url)
	var req = initRequest(url);
	req.onreadystatechange = function() {
		if (req.readyState == 4) {
...
function actionResourceTermination(id, action_id, status) {
	swal({
		...
		if (isConfirm) {
			...
			// alert(id);
			// alert(action_id);
			var url = 'accauth/actionResourceTermination.action?id=' + id
					+ '&actionId=' + action_id + '&actionResourceStatus='
					+ status;
			var req = initRequest(url);
			// alert(url);
			req.onreadystatechange = function() {
				if (req.readyState == 4) {
					if (req.status == 200) {
						...
						swal("Operation!", "Operation Successfully....",
								"success");
					} else {
						swal("Sorry Operation Not Done", "Operation not done ",
								"error");
					}
				}
			};
...
function getPresentTimeYear() {
	...
	//
	// invoiceDate = new
	// dhtmlXCalendarObject(["invoiceStartDateOver","invoiceEndDateOver","invoiceStartDate","invoiceEndDate"]);
...
function generateInvoice() {
	...
	// var
	// invoiceStartDateOver=document.getElementById("invoiceStartDateOver").value;
	// var
	// invoiceEndDateOver=document.getElementById("invoiceEndDateOver").value;
	var cheked;
	cheked = ($('#resourceAll').is(":checked"));
	// alert(cheked)
	if (isvalidationInvoice(invoiceMonth, invoiceYear, invoiceResource, cheked)) {
		var url = CONTENXT_PATH + '/sag/generateInvoice.action?invoiceMonth='
				+ invoiceMonth + '&invoiceYear=' + invoiceYear
				+ '&invoiceResource=' + invoiceResource + '&cheked=' + cheked;
		// alert(url)
		var req = initRequest(url);
		req.onreadystatechange = function() {
			if (req.readyState == 4) {
				if (req.status == 200) {
					...
					if (req.responseText == 1) {
						$("invoiceGenerarionMessage").html(
								" <font color='green'>"
										+ "Successfully Invoice Generated."
										+ "</font>").show().delay(5000)
								.fadeOut();
						// alert("success")
						...
						$("invoiceGenerarionMessage").html(
								"<font color='red'>"
										+ "SOW Need To be Approved."
										+ "</font>").show().delay(5000)
								.fadeOut();
					} else if (req.responseText == -2) {
						$("invoiceGenerarionMessage").html(
								"<font color='red'>" + "User Not Exist."
										+ "</font>").show().delay(5000)
								.fadeOut();
						...
						$("invoiceGenerarionMessage").html(
								" <font color='red'>"
										+ "Invoice Already Exist." + "</font>")
								.show().delay(5000).fadeOut();
						// alert("invoice already exist")
					} else if (req.responseText == -5) {
						$("invoiceGenerarionMessage").html(
								" <font color='red'>" + "Time Sheet not Found"
										+ "</font>").show().delay(5000)
								.fadeOut();
...
function isvalidationInvoice(invoiceMonth, invoiceYear, invoiceResource, cheked) {
	...
	if (invoiceYear == "") {
		...
		$("invoiceGenerarionMessage").html(
				" <font color='red'>Please enter the Year</font>.").show()
				.delay(5000).fadeOut();
		return false;
	} else {
		// alert("inn else ")
	...
	if (yyyy >= invoiceYear) {
		if (mm >= invoiceMonth) {
			...
			$("invoiceGenerarionMessage").html(
					" <font color='red'>Month Not Valid</font>.").show().delay(
					5000).fadeOut();
			return false;
		}
	} else {
		$("invoiceGenerarionMessage").html(
				" <font color='red'>Year Not Valid</font>.").show().delay(5000)
				.fadeOut();
		return false;
	}
	if (cheked) {
		...
		if (re.test(invoiceResource)) {
			...
			$("invoiceGenerarionMessage").html(
					" <font color='red'>Must be valid email</font>.").show()
					.delay(5000).fadeOut();
			return false;
		}
	}
...
function getBalanceAmt() {
	...
	if (invoiceStatus == 'Paid') {
		...
		if (transNO == "") {
			$("errorMsg")
					.html(
							" <font color='red'>Please Enter Transaction Number</font>");
			return false;
		}
		if (paid == 0 || paid == "") {
			// alert("pj")
			$("errorMsg").html(
					"<font color='red'>Please Enter Paid Amount</font>");
			return false;
		}
			...
			$("errorMsg")
					.html(
							" <font color='red'>Paid Amount Should be Less then with Total.</font>");
			return false;
		} else {
			$("errorMsg").html("");
			...
			.keyup(
					function() {
						...
						if (el.val().length >= 250) {
							...
							$("#addingAccAuthValid").text(
									250 - el.val().length
											+ ' Characters remaining . ');
							$("#addingAccAuthValid").show().delay(5000)
									.fadeOut();
						...
						if (el.val().length == 250) {
							...
							$("#addingAccAuthValid")
									.text(
											' Cannot enter  more than 250 Characters .');
							$("#addingAccAuthValid").show().delay(5000)
									.fadeOut();
...
function regStateChange(id1) {
	...
	if (id == -1) {
		$("#regValidation").html(
				" <font color=red>Country field is Required</font>.");
...
function regStateChanging(data, id1) {
	...
	if (fieldId == 'country') {
		$select = $('#state1');
	} else if (fieldId == 'consultantPerCountry') {
		$select = $('#consultantPerState');
	} else if (fieldId == 'consultantCurCountry') {
		$select = $('#consultantCurState');
	}
...
function orgNameCheck(textBoxId, errorTextId) {
	...
	if ($(textBoxId).val() == '') {
		$(errorTextId).html(
				'<span><font color=red>Please enter name</font></span>');
	...
	$
			.ajax({
				type : 'POST',
				url : CONTENXT_PATH + '/general/orgNameCheck?accountNameCheck='
						+ $(textBoxId).val(),
				dataType : 'text',
				success : function(data, stat, xhr) {
					console.log('RESPONSE SAYS ' + data + " "
							+ xhr.getResponseHeader('exists'));
					if (xhr.getResponseHeader('exists') === 'free'
						...
						if ($(textBoxId).val() != '') {
							...
							$(errorTextId)
									.html(
											'<span><font color=red>This name already exists</font></span>');
						} else {
							$(errorTextId)
									.html(
											'<span><font color=red>Please enter name</font></span>');
				...
				error : function(data, stat, xhr) {
					// console.log('RESPONSE SAYS '+data+" " +
					// xhr.getResponseHeader('urlexists'));
					$(textBoxId).css('border', '1px solid red');
...
function orgWebAddressCheck(textBoxId, errorTextId) {
	...
	if ($(textBoxId).val() == '') {
		$(errorTextId).html(
				'<span><font color=red>Please enter URL</font></span>');
		$(textBoxId).css('border', '1px solid red');
		setTimeout(function() {
			$(textBoxId).css('border', '');
			$(errorTextId).children().remove();
	...
	$
			.ajax({
				type : 'POST',
				url : CONTENXT_PATH
						+ '/general/orgWebAddressCheck?accountURLCheck='
						+ $(textBoxId).val(),
				dataType : 'text',
				success : function(data, stat, xhr) {
					console.log('RESPONSE SAYS ' + data + " "
					...
					if (xhr.getResponseHeader('urlexists') === 'free'
						...
						if ($(textBoxId).val() != '') {
							...
							$(errorTextId)
									.html(
											'<span><font color=red>This Url already exists</font></span>');
						} else {
							$(errorTextId)
									.html(
											'<span><font color=red>Please enter URL</font></span>');
...
function regOfficeEmailValidation() {
	...
	if (!re.test(email)) {
		// $("#addContactError").html("");
		$("#office_emailIdSpan").html(
				" <font color='red'>Must be valid corp email</font>.");
		$("#office_emailId").css("border", "1px solid red");
		setTimeout(function() {
			$("#office_emailId").css('border', '');
			$("#office_emailIdSpan").children().remove();
		}, 3000);
		return false;
	} else {
		...
		if (ext == '') {
			$("#office_emailIdSpan")
					.html(
							" <font color='red'>Please Enter Mail Extension first!!!</font>.");
		...
		if (ext != office_email_ext) {
			$("#office_emailIdSpan")
					.html(
							" <font color='red'>Email extension must be same as specified above</font>.");
			$("#office_emailId").css("border", "1px solid red");
			document.getElementById("office_emailId").value = "";
			setTimeout(function() {
				$("#office_emailId").css('border', '');
		...
		var url = CONTENXT_PATH
				+ '/general/officeEmailCheck.action?ContactEmail=' + email;
		// alert(url)
		var req = initRequest(url);
		req.onreadystatechange = function() {
			if (req.readyState == 4) {
				if (req.status == 200) {
					...
					if (req.responseText == "success") {
						$("#office_emailIdSpan")
								.html(
										"  <font color='green'>E-mail is Available</font>");
						$("#office_emailId").css("border", "1px solid green");
						setTimeout(function() {
							$("#office_emailId").css('border', '');
							$("#office_emailIdSpan").children().remove();
						...
						$("#office_emailIdSpan")
								.html(
										"<font color=#B20000>Email Id  Already Exists !</font>");
						$("#office_emailId").css("border", "1px solid red");
						setTimeout(function() {
							$("#office_emailId").css('border', '');
							$("#office_emailIdSpan").children().remove();
...
function getTotalHoursForTooltip(usr_id) {
	...
	var url = CONTENXT_PATH + '/sag/getTotalHoursTooltip.action?invoiceMonth='
			+ month + '&invoiceYear=' + Year + '&usrId=' + usr_id;
...
function setTotalHours(response) {
	...
	if (response.length > 0) {
		...
		for (var i = 0; i < techReviewList.length - 1; i++) {
			...
			{
				var row = $("<tr/>")
				$("#hoursOverlayTable").append(row); // this will append tr
				// element to table...
				// keep its reference
				// for a while since we
				// will add cels into it
				// row.append($('<td><a
				// href=editVendorTierDetails.action?VendorTierId='+Values[4]+">"
				// + Values[0] + "</a></td>"));
		...
		row
				.append($('<td colspan="5"><font style="color: red;font-size: 15px;">No Records to display</font></td>'))
	}
	pager.init();
...
function userRegFillAddress() {
	if (document.getElementById("checkAddress").checked == true) {
		document.getElementById("address1").value = document
				.getElementById("org_address1").value;
		document.getElementById("address1").disabled = true;
		document.getElementById("address2").value = document
				.getElementById("org_address2").value;
		document.getElementById("address2").disabled = true;
		document.getElementById("city").value = document
				.getElementById("org_city").value;
		document.getElementById("city").disabled = true;
		document.getElementById("country").value = document
				.getElementById("org_country").value;
		document.getElementById("country").disabled = true;
		var $options = $("#org_state > option").clone();
		$('#state1').append($options);
		document.getElementById("state1").value = document
				.getElementById("org_state").value;
		document.getElementById("state1").disabled = true;
		document.getElementById("zip").value = document
				.getElementById("org_zip").value;
		document.getElementById("zip").disabled = true;
		document.getElementById("fax").value = document
				.getElementById("org_fax").value;
...
// for filling currenet address
function consultantRegFillAddress() {
	if (document.getElementById("checkAddress").checked == true) {
		document.getElementById("consultantCurAddress1").value = document
				.getElementById("consultantPerAddress1").value;
		document.getElementById("consultantCurAddress1").disabled = true;
		document.getElementById("consultantCurAddress2").value = document
				.getElementById("consultantPerAddress2").value;
		document.getElementById("consultantCurAddress2").disabled = true;
		document.getElementById("consultantCurCity").value = document
				.getElementById("consultantPerCity").value;
		document.getElementById("consultantCurCity").disabled = true;
		document.getElementById("consultantCurCountry").value = document
				.getElementById("consultantPerCountry").value;
		document.getElementById("consultantCurCountry").disabled = true;
		var $options = $("#consultantPerState > option").clone();
		$('#consultantCurState').append($options);
		document.getElementById("consultantCurState").value = document
				.getElementById("consultantPerState").value;
		document.getElementById("consultantCurState").disabled = true;
		document.getElementById("consultantCurZip").value = document
				.getElementById("consultantPerZip").value;
		document.getElementById("consultantCurZip").disabled = true;
	}
	if (document.getElementById("checkAddress").checked == false) {
		document.getElementById("consultantCurAddress1").disabled = false;
		document.getElementById("consultantCurAddress1").value = '';
		document.getElementById("consultantCurAddress2").disabled = false;
		document.getElementById("consultantCurAddress2").value = '';
		document.getElementById("consultantCurCity").disabled = false;
		document.getElementById("consultantCurCity").value = '';
		document.getElementById("consultantCurCountry").disabled = false;
		document.getElementById("consultantCurCountry").value = '';
		document.getElementById("consultantCurState").disabled = false;
		document.getElementById("consultantCurState").value = '';
		document.getElementById("consultantCurZip").disabled = false;
		document.getElementById("consultantCurZip").value = '';
	}
}
...
function getValidMailExtention() {
	...
	if (mailExtention.match(emailExp)) {
		// alert("fine")
		var url = CONTENXT_PATH
				+ "/general/mailextensionCheck.action?email_ext="
				+ mailExtention;
		// alert("url-->"+url);
		var req = initRequest(url);
		req.onreadystatechange = function() {
			if (req.readyState == 4) {
				if (req.status == 200) {
					...
					if (req.responseText == "Not Exist") {
						$("#orgExtCheckSpan")
								.html(
										"  <font color='green'><br>Mail Extension is Available</font>");
						...
						$("#orgExtCheckSpan")
								.html(
										" <font color=red><br>Mail Extension Already Exists !</font>");
		...
		$("#orgExtCheckSpan")
				.html(
						"  <font color='red'><br>Please enter valid email extension!</font>");
...
function doLogoutOrNot() {
	...
	if (confirmation == true) {
		// alert("In if
		// cond-->-->"+confirmation+"-->contextPath"+CONTENXT_PATH);
...
function acceptNumbersOnly(evt) {
	...
	if (iKeyCode != 46 && iKeyCode > 31 && (iKeyCode < 48 || iKeyCode > 57)) {
		$("errorMsg").html("<font color='red'>Enter Numbers only</font>");
		return false;
	} else {
...
function downloadXLSPendingInoviceList() {
	...
	// alert(gridDownload);
	var url = CONTENXT_PATH
			+ "/recruitment/consultant/downloadXlsResults.action?pdfHeaderName=Pending payments List&gridDownload="
...
function sendEmailOfInvoice(invCreatedBy) {
	...
	var specialBox = document
			.getElementById('sendEmailOfPendingInvoiceOverlay');
	if (specialBox.style.display == "block") {
		specialBox.style.display = "none";
	} else {
		specialBox.style.display = "block";
...
// Email Id Checked whether it is there or not
function consultantEmailIdCheck() {

	var consultantEmail = document.getElementById("consultantEmail").value;
	// alert("consultantEmail-->"+consultantEmail);

	re = /[^@]+@[^@]+\.[a-zA-Z]{2,6}/;
	if (!re.test(consultantEmail)) {

		$("#consultantEmailspan").html(
				" <font color='red'>Must be valid email</font>.");
		...
		var url = CONTENXT_PATH
				+ '//users/general/consultantEmailIdCheck.action?consultantEmailCheck='
				+ consultantEmail;
		...
		req.onreadystatechange = function() {
			if (req.readyState == 4) {
				if (req.status == 200) {
					...
					consultantResultEmail(req.responseText);
...
function consultantResultEmail(response) {
	// alert("response -->" + response)
	if (response == "exist") {

		var consultantEmail = document.getElementById('consultantEmail').value;
		document.getElementById('consultantEmail').value = "";
		document.getElementById('existedEmail').innerHTML = consultantEmail;
		document.getElementById('existedEmailId').value = consultantEmail;
...
function forgotPasswordLink(emailId, resultMessage, loadingMessage) {
	forgotPassword(emailId, resultMessage, loadingMessage);
}

function setValueOnLoginForm(flagValue) {
	// alert(flagValue);
	if (flagValue == 5) {
		alert("flag value is " + flagValue)
		$('.customerRecaptchaDiv').hide();
		$('.vendorRecaptchaDiv').show();
	} else if (flagValue == 1) {
		$('.vendorRecaptchaDiv').hide();
		$('.customerRecaptchaDiv').show();
	} else {
		$('.vendorRecaptchaDiv').hide();
		$('.customerRecaptchaDiv').hide();
	}
	document.getElementById('customerOrVenFlag').value = flagValue;
}

function captchaValidation(captchaValue) {
	var response = false;
	// alert(captchaValue);
	var url = CONTENXT_PATH + "/captchaValidation.action?recaptchaResponse="
			+ captchaValue;
	// alert("url-->"+url);
	var req = initRequest(url);
	req.onreadystatechange = function() {
...
function conForgotPassword(emailId, resultMessage, loadingMessage) {
	var res = false;
	res = conForgotPasswordRecaptchaValidation();
	if (res === true) {
		forgotPassword(emailId, resultMessage, loadingMessage);
	} else {
		$('#' + resultMessage).html("<font color=red>Invalid Captcha</font>");
		return false;
	}

}
function custVenForgotPassword(emailId, resultMessage, loadingMessage) {

	var response = false;
	response = custVenForgotPasswordRecaptchaValidation();
	// alert(response);
	if (response === true) {
		forgotPassword(emailId, resultMessage, loadingMessage);
	} else {
...
function conFpassInReg(emailId, resultMessage, loadingMessage) {
	var response = false;
	response = conFPassRecaptchaValidation();
	// alert(response);
	if (response === true) {
		forgotPasswordLink(emailId, resultMessage, loadingMessage);
	} else {
		return false;
	}
}

function forgotPasswordCaptchaValidation() {
	// alert("helloe");
	var result = false;
	var captchaValue = null;
	captchaValue = document.getElementById("g-recaptcha-response").value;
	// alert(captchaValue);
	result = captchaValidation(captchaValue);
	// alert("In consultant validation " + result);
	if (result === false) {
		$("#captchaSpan").html("<font color=red><br>Invalid captcha..</font>");
		setTimeout(function() {
			$("#captchaSpan").children().remove();
		}, 3000);
		return false;
	}
}
function setRatecardValues(id, jobTitle, minRate, maxRate, comments) {
	// alert(id + "" + jobTitle + "" + minRate + "" + maxRate + "" + comments);
	document.getElementById("overlayRatecardId").value = id;
	document.getElementById("overlayJobTitle").value = jobTitle;
	document.getElementById("overlayMinRate").value = minRate;
	document.getElementById("overlayMaxRate").value = maxRate;
	document.getElementById("overlayComments").value = comments;
}
function addOrUpdateRatecardChecking(flag) {
	// alert(flag);
	if (flag == 'a') {
		// alert("add");
		var update = document.getElementById("updateDiv");
		...
		document.getElementById("overlayHeading").innerHTML = "&nbsp;&nbsp;Add&nbsp;Ratecard";
	}
	if (flag == 'u') {
		// alert("Updatew");
		var add = document.getElementById("addDiv");
		...
		document.getElementById("overlayHeading").innerHTML = "&nbsp;&nbsp;Update&nbsp;Ratecard";

	}
}
function clearRatecardValuesForAdd() {
	// alert("clearActionValues");
	document.getElementById("overlayRatecardId").value = "";
	document.getElementById("overlayJobTitle").value = "";
	document.getElementById("overlayMinRate").value = "";
	document.getElementById("overlayMaxRate").value = "";
	document.getElementById("overlayComments").value = "";
	var add = document.getElementById("addDiv");
	add.style.display = "block";

}
function insertOrUpdateRatecard(flag) {
	
	
	var overlayRatecardId = document.getElementById("overlayRatecardId").value;
	var overlayJobTitle = document.getElementById("overlayJobTitle").value;
	var overlayMinRate = document.getElementById("overlayMinRate").value;
	var overlayMaxRate = document.getElementById("overlayMaxRate").value;
	var overlayComments = document.getElementById("overlayComments").value;
	// var status=document.getElementById("status").value;
	//alert(overlayRatecardId);
	// alert(accauthStatus);
	// alert(addingAccAuthDesc);
	// alert(actionId);
	if(overlayJobTitle.trim()==""){
		document.getElementById("validationMessage").innerHTML=" <font color='red'>Please Enter Job Title</font>";
		return false;
	}
	if(overlayMinRate.trim()==""){
		document.getElementById("validationMessage").innerHTML=" <font color='red'>Please Enter Minimum Rate</font>";
		return false;
	}
	if(overlayMaxRate.trim()==""){
		document.getElementById("validationMessage").innerHTML=" <font color='red'>Please Enter Maximum Rate</font>";
		return false;
	}
	if(overlayComments.trim()==""){
		document.getElementById("validationMessage").innerHTML=" <font color='red'>Please Enter Comments</font>";
		return false;
	}
	else{
	document.getElementById("loadingRateCardSearch").style.display="block";
	initSessionTimer();
	...
	var url = CONTENXT_PATH
			+ '/Requirements/insertOrUpdateRatecard.action?ratecardId='
			+ overlayRatecardId + '&jobTitle=' + overlayJobTitle + '&minRate='
			+ overlayMinRate + '&maxRate=' + overlayMaxRate + '&comments='
			+ overlayComments;
	...
	req.onreadystatechange = function() {
		if (req.readyState == 4) {
			...
			if (req.status == 200) {
				...
				//alert(req.responseText);
				// document.getElementById("validationMessage").innerHTML=req.responseText;
				// window.location = "getAccAuthrization.action?resultMessage="+
				 document.getElementById("validationMessage").innerHTML="<font color='green'>" + req.responseText + "</font>."
...
function reloadRateCardPage(){
	window.location = CONTENXT_PATH+"/Requirements/getRateCardList.action";
}
function rateValidation(rate){
	var regex=/^[0-9]+$/;
    if (!rate.value.match(regex))
    {
    	document.getElementById("validationMessage").innerHTML=" <font color='red'>Please enter numbers only</font>";
    	rate.value="";
        return false;
    }
}
