...
function forwardReviewToCustomer(){
    ...
    else
    {
        ...
        if(techieQuestions<1)
        {
            ...
            $("e").html(" <font color='red'>Please give number of Questions</font>");
           // $("e").show().delay(4000).fadeOut();
            return false;
        }
         else{
...
function getSearchTechReviewList(){
   ...
   // alert(interviewType);
    ...
    var url='.././Requirements/getSearchTechReviewList.action?reviewStartDate='+reviewStartDate+'&techReviewStatus='+techReviewStatus+'&reviewEndDate='+reviewEndDate+'&interviewType='+interviewType;
    var req=initRequest(url);
    req.onreadystatechange = function() {
    ...
    if(response.length>0){
        ...
        for(var i=0;i<techReviewList.length-1;i++){   
            ...
            var Values=techReviewList[i].split("|");
                ...
                row.append($('<td><a href=../../Requirements/requirementedit.action?customerFlag=customer&accountFlag=techReviews&jdId='+Values[16]+'&accountSearchID='+Values[17]+'&requirementId='+Values[9]+'>' + Values[16] + "</a></td>"));
                ...
                {	
                	if(Values[19]=="false")
                	{
                		row.append($("<td><figcaption><button style='cursor: default' type='button' onclick='sendNotificationToConsultant();'>Send</button></figcaption></td>"));
                	}
                	else
                	{
                		row.append($("<td><figcaption><button style='cursor: default' type='button' disabled>send</button></figcaption></td>"));
                	}
                }
                
            else if(Values[18]==13||Values[18]==1||Values[18]==3)
            	{
            		if(Values[19]=="false")
            		{
            			row.append($("<td><figcaption><button style='cursor: default' type='button' onclick='sendNotificationToVendor();'>send</button></figcaption></td>"));
            		}
            		else
            		{
            			row.append($("<td><figcaption><button style='cursor: default' type='button' disabled>send</button></figcaption></td>"));
...
function doTechReviewAttachmentDownload(acc_attachment_id)
{
    ...
    window.location = './recruitment/consultant/downloadAttachmentTechReview.action?pageFlag='+pageFlag+'&acc_attachment_id='+acc_attachment_id+'&techSearch='+techSearch+'&reviewStartDate='+reviewStartDate+'&techReviewStatus='+techReviewStatus+'&reviewEndDate='+reviewEndDate;
}
...
function acceptNumbers(evt){
    ...
    {
        $("#skillValid").html(" <font color='red'>Enter Numbers only</font>");
        $("#skillValid").show().delay(3000).fadeOut();
        return false;
    }
    else
    {
...
function sendNotificationToVendor() {
	...
	swal({
		 title: "Do you want to Notify the Vendor ?",
...
function sendNotificationToConsultant() {
	
	
	swal({
		 title: "Do you want to Notify the Consultant ?",
