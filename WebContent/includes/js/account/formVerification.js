function validateForm1() {
	...
	if (account_type == "") {
		// alert(account_type);
		$("#resultMessage")
				.html("<font color='red'>Select account type</font>");
		$("#resultMessage").show().delay(5000).fadeOut();
		return false;
	}
	// alert(email_ext)
	if (email_ext == "") {
		// alert(email_ext);
		$("#resultMessage").html(
				"<font color='red'>Enter email extension</font>");
		$("#resultMessage").show().delay(5000).fadeOut();
		return false;
	}
	return true;
...
function validateDropDown(id, validId) {
	...
	{
		// document.getElementById(validId).innerHTML = "Please make a
		// selection";
		isValid = false;
	} else {
...
function alphanumeric() {
	var region = document.getElementById("acc_reqion"); // acc_reqion-->ID of
	// textbox
	var taxID = document.getElementById("acc_tax_id"); // acc_reqion-->ID of
	// textbox
	var alphanum = /^[0-9a-bA-B]+$/; // This contains A to Z , 0 to 9 and A
	// to B
...
function urlCheck(textBoxId, errorTextId) {
	...
	$
			.ajax({
				type : 'POST',
				url : CONTENXT_PATH
						+ '/acc/ajaxAccountURLCheck?accountURLCheck='
						+ $(textBoxId).val(),
				dataType : 'text',
				success : function(data, stat, xhr) {
					...
					if (xhr.getResponseHeader('urlexists') === 'free'
						...
						if ($(textBoxId).val() != '') {
							$(errorTextId).html(
									'<span>This Url already exists</span>');
						} else {
							$(errorTextId).html(
									'<span>You must enter an URL</span>');
						}
...
function nameCheck(textBoxId, errorTextId) {
	if ($(textBoxId).val() == '') {
		$(errorTextId).html('<span>You must enter name</span>');
		$(textBoxId).css('border', '1px solid red');
		setTimeout(function() {
			$(textBoxId).css('border', '');
			$(errorTextId).children().remove();
	...
	$.ajax({
		type : 'POST',
		url : CONTENXT_PATH + '/acc/ajaxAccountNameCheck?accountNameCheck='
				+ $(textBoxId).val(),
		dataType : 'text',
		success : function(data, stat, xhr) {
			console.log('RESPONSE SAYS ' + data + " "
			...
			if (xhr.getResponseHeader('exists') === 'free'
				...
				if ($(textBoxId).val() != '') {
					$(errorTextId)
							.html('<span>This name already exists</span>');
				} else {
					$(errorTextId).html('<span>You must enter name</span>');
				}
				$(textBoxId).css('border', '1px solid red');
...
function getValidExtention() {
	...
	{
		// alert("fine")
		var url = CONTENXT_PATH + "/acc/mailext_Check.action?email_ext="
				+ mailExtention;
		// alert("url-->"+url);
	...
	// else{
	// $("#orgExtCheckSpan").html(" <font color='red'><br>Please enter valid
	// email extension!</font>");
	//           
	// document.getElementById("email_ext").value="";
...
function cityValidate(event) {
	...
	if ((inputValue > 32 && inputValue < 48 || inputValue > 57
			&& inputValue < 65 || inputValue > 90 && inputValue < 97 || inputValue > 122
			&& inputValue < 128)
			&& (inputValue != 32)) {
...
function noeValidate(event) {
	...
	var inputValue = (event.which) ? event.which : event.keyCode
	if ((inputValue > 32 && inputValue < 48 || inputValue > 57
			&& inputValue < 91 || inputValue > 90 && inputValue < 123 || inputValue > 122
			&& inputValue < 128)
			&& (inputValue != 32)) {
		$('#employeeValidation').css("color", "red");
...
function custVenRegValidation() {

	var orgName = $("#orgName").val();
	var org_web_address = $("#org_web_address").val();
	var noOfEmployees = $("#noOfEmployees").val();
	var email_ext = $("#email_ext").val();

	var first_name = $("#first_name").val();
	var last_name = $("#last_name").val();

	var office_Phone = $("#office_Phone").val();
	var office_emailId = $("#office_emailId").val();
	...
	if (orgName == "") {
		...
		$("#orgNameCheckSpan").html("<font color='red'>Enter Name</font>");
	...
	if (org_web_address == "") {
		...
		$("#orgWebCheckSpan")
				.html("<font color='red'>Enter Website URL</font>");
		$("#org_web_address").css("border", "1px solid red");
		return false;
	}

	if (email_ext == "") {

		$("#orgExtCheckSpan").html(
				"<font color='red'>Enter Mail Extension</font>");
		$("#email_ext").css("border", "1px solid red");
		return false;
	}
	...
	if (first_name == "") {
		...
		$("#first_nameSpan").html("<font color='red'>Enter First Name</font>");
	...
	if (last_name == "") {
		...
		$("#last_nameSpan").html("<font color='red'>Enter Last Name</font>");
	...
	if (office_Phone == "") {
		...
		$("#office_PhoneSpan").html(
				"<font color='red'>Enter Office Phone Number</font>");
	...
	if (office_emailId == "") {
		...
		$("#office_emailIdSpan").html(
				"<font color='red'>Enter Office Email-Id</font>");
	...
	if (address1 == "") {
		...
		$("#address1Span").html("<font color='red'>Enter Address</font>");
	...
	if (city == "") {
		...
		$("#citySpan").html("<font color='red'>Enter City</font>");
	...
	if (country == -1) {
		...
		$("#countrySpan").html("<font color='red'>Select Country</font>");
...
function clearErrorMsg(userRegForm) {

	if (userRegForm.id == "orgName") {
		$("#orgNameCheckSpan").html("");
		$("#orgName").css("border", "1px solid #ddd");
	}
	if (userRegForm.id == "org_web_address") {
		$("#org_web_address").css("border", "1px solid #ddd");
		$("#orgWebCheckSpan").html("");
	}
	if (userRegForm.id == "email_ext") {
		$("#email_ext").css("border", "1px solid #ddd");
		$("#orgExtCheckSpan").html("");
	}
	if (userRegForm.id == "first_name") {
		$("#first_name").css("border", "1px solid #ddd");
		$("#first_nameSpan").html("");
	}
	if (userRegForm.id == "last_name") {
		$("#last_name").css("border", "1px solid #ddd");
		$("#last_nameSpan").html("");
	}
	if (userRegForm.id == "office_Phone") {
		$("#office_Phone").css("border", "1px solid #ddd");
		$("#office_PhoneSpan").html("");
	}
	if (userRegForm.id == "office_emailId") {
		$("#office_emailId").css("border", "1px solid #ddd");
		$("#office_emailIdSpan").html("");
	}
	if (userRegForm.id == "address1") {
		$("#address1").css("border", "1px solid #ddd");
		$("#address1Span").html("");
	}
	if (userRegForm.id == "city") {
		$("#city").css("border", "1px solid #ddd");
		$("#citySpan").html("");
	}
	if (userRegForm.id == "country") {
		$("#country").css("border", "1px solid #ddd");
		$("#countrySpan").html("");
	}
	if (userRegForm.id == "state1") {
		$("#state1").css("border", "1px solid #ddd");
		$("#state1Span").html("");
	}
...
function accountTypeDropDown() {
	...
	if ($('#account_type').val() == 5) {
		...
		document.getElementById("vspecial").style.display = "block";
		document.getElementById("vnotes").style.display = "block";
		// vendorHidden= false;

	} else if ($('#account_type').val() == "") {

		document.getElementById("vspecial").style.display = "none";
		document.getElementById("vnotes").style.display = "none";
		document.getElementById("cnotes").style.display = "none";
